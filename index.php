...
if ($keyword !== '') {
    $stmt = $pdo->prepare("SELECT * FROM todolist WHERE user_id = ? AND title LIKE ? ORDER BY created_at DESC");
    $stmt->execute([$_SESSION['user_id'], '%' . $keyword . '%']);
}// タスク取得 
else {
    $stmt = $pdo->prepare("SELECT * FROM  WHERE user_id = ? ORDER BY created_at DESC");
